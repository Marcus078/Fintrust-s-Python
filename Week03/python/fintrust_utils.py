...
def validate_id_number(id_number):
    """Return True if ID number is 13 digits and numeric."""
    s = str(id_number)
    return len(s) == 13 and s.isdigit()


def validate_account_type(account_type):
    """Return True if account type is valid."""
    return account_type in ("savings", "cheque", "credit")
...
def calculate_monthly_fee(account_type):
    """Return monthly fee based on account type."""
    fees = {
        "savings": 0.00,
        "cheque": 65.00,
        "credit": 120.00
...
def categorise_transaction(amount):
    """Categorise a transaction by amount."""
    amount = abs(amount)
